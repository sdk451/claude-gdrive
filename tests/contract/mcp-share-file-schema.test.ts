...
function initializeBody(id: number | string) {
  return {
    ...
    params: {
      ...
      capabilities: {},
      clientInfo: { name: "vitest-share-file-schema", version: "0.0.0" },
    },
  };
}

describe("TOK-32 share_file inputSchema (contract)", () => {
  it("tools/list exposes share_file with action enum, optional grant/revoke fields, destructiveHint", async () => {
    const app = createApp();
    ...
    const initRes = await app.request("http://localhost/mcp", {
      ...
      body: JSON.stringify(initializeBody("tok-32-schema")),
    });
    ...
    const tool = body.result?.tools?.find((t) => t.name === "share_file");
    ...
    expect(props?.granteeType).toBeDefined();
    expect(props?.emailAddress).toBeDefined();
    expect(props?.domain).toBeDefined();
    expect(props?.permissionId).toBeDefined();
    const req = schema?.required as string[];
    expect(req.includes("action")).toBe(true);
