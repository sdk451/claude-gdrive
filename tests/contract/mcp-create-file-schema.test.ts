...
function initializeBody(id: number | string) {
  return {
    ...
    params: {
      ...
      capabilities: {},
      clientInfo: { name: "vitest-create-file-schema", version: "0.0.0" },
    },
  };
}

describe("TOK-28 create_file inputSchema (contract)", () => {
  it("tools/list exposes create_file with required name and mimeType", async () => {
    const app = createApp();
    ...
    const initRes = await app.request("http://localhost/mcp", {
      ...
      body: JSON.stringify(initializeBody("tok-28-schema")),
    });
    ...
    expect(props?.parentFolderId).toBeDefined();
    expect(props?.mediaBase64).toBeDefined();
    expect(props?.mediaMimeType).toBeDefined();
    const req = schema?.required as string[];
    expect(req.includes("name")).toBe(true);
    expect(req.includes("mimeType")).toBe(true);
    expect(req.includes("parentFolderId")).toBe(false);
