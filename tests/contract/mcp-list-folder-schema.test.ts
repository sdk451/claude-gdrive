...
function initializeBody(id: number | string) {
  return {
    ...
    params: {
      ...
      capabilities: {},
      clientInfo: { name: "vitest-list-folder-schema", version: "0.0.0" },
    },
  };
}

describe("TOK-33 list_folder inputSchema (contract)", () => {
  it("tools/list exposes list_folder with required folderId, optional pagination, readOnlyHint", async () => {
    const app = createApp();
    ...
    const initRes = await app.request("http://localhost/mcp", {
      ...
      body: JSON.stringify(initializeBody("tok-33-schema")),
    });
    ...
    const tool = body.result?.tools?.find((t) => t.name === "list_folder");
    ...
    expect(req.includes("folderId")).toBe(true);
  });
});
