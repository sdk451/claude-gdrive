...
function initializeBody(id: number | string) {
  return {
    ...
    params: {
      ...
      capabilities: {},
      clientInfo: { name: "vitest-metadata-schema", version: "0.0.0" },
    },
  };
}

describe("TOK-26 get_file_metadata inputSchema (contract)", () => {
  it("tools/list exposes get_file_metadata with required fileId", async () => {
    const app = createApp();
    ...
    const initRes = await app.request("http://localhost/mcp", {
      ...
      body: JSON.stringify(initializeBody("tok-26-schema")),
    });
    ...
    const meta = body.result?.tools?.find((t) => t.name === "get_file_metadata");
