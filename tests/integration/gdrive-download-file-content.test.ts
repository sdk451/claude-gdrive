...
function initializeBody(id: number | string) {
  return {
    ...
    params: {
      ...
      capabilities: {},
      clientInfo: { name: "vitest-download-file", version: "0.0.0" },
    },
  };
}

describe("TOK-25 download_file_content integration (stubbed Drive)", () => {
  it("tools/call download_file_content without stub returns isError (no token yet)", async () => {
    const app = createApp();
    ...
    const initRes = await app.request("http://localhost/mcp", {
      ...
      body: JSON.stringify(initializeBody("tok-25-no-token")),
    });
    ...
    const callRes = await app.request("http://localhost/mcp", {
      ...
      body: JSON.stringify({
        ...
        id: 30,
        method: "tools/call",
        params: { name: "download_file_content", arguments: { fileId: "pdf-1" } },
      }),
    });
  ...
  it("tools/call download_file_content forwards fileId to stub", async () => {
    const downloads: DownloadFileContentParams[] = [];
    const stub: DriveFilesPort = {
      ...
      async downloadFileContent(params) {
    ...
    const initRes = await app.request("http://localhost/mcp", {
      ...
      body: JSON.stringify(initializeBody("tok-25-int")),
    });
    expect(initRes.status).toBe(200);
    const sessionId = initRes.headers.get("mcp-session-id")!;
    const negotiated = ((await initRes.json()) as { result: { protocolVersion: string } }).result
    ...
    const callRes = await app.request("http://localhost/mcp", {
      ...
      body: JSON.stringify({
        ...
        id: 31,
        method: "tools/call",
        params: { name: "download_file_content", arguments: { fileId: "abc" } },
      }),
    });
    expect(callRes.status).toBe(200);
    const body = (await callRes.json()) as {
    ...
    expect(parsed.base64).toContain("JVBER");

    expect(downloads).toHaveLength(1);
    expect(downloads[0]?.fileId).toBe("abc");
  });
});
